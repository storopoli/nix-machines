{ pkgs, config, username, lib, inputs, ... }:
...
in {
  ...
  config.networking.firewall.allowedTCPPorts = [ 80 443 8448 6167 8080 ];
  ...
  options.services.matrix-conduit.settings = lib.mkOption {
    apply = old:
      old // (if (old.global ? "unix_socket_path") then {
        global = builtins.removeAttrs old.global [ "address" "port" ];
      } else
        { });
  };
  ...
  # manually.
  config.systemd.services.conduit.serviceConfig.RestrictAddressFamilies =
    [ "AF_UNIX" ];
}
